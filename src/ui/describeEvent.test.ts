...
describe('describeEvent', () => {
  it('names the side for a nursing session', () => {
    const described = describeEvent(nursing(NOW, 15 * MINUTE_MS, 'right'), 'ml', NOW)
  ...
  it('shows a bottle in the user’s chosen unit', () => {
    ...
    expect(describeEvent(event, 'ml', NOW).detail).toBe('120 ml')
    expect(describeEvent(event, 'oz', NOW).detail).toBe('4.1 oz')
  })

  it('distinguishes breast milk from formula', () => {
    expect(describeEvent(bottle(NOW, 120, 'breast_milk'), 'ml', NOW).title).toBe(
      'Bottle, breast milk',
    )
    expect(describeEvent(bottle(NOW, 120, 'formula'), 'ml', NOW).title).toBe(
      'Bottle, formula',
    )
  })
  ...
  it('labels a finished sleep with its length and wake time', () => {
    ...
    expect(describeEvent(nap, 'ml', NOW)).toMatchObject({
      category: 'sleep',
      title: 'Nap',
  ...
  it('marks a running sleep as live and counts up to now', () => {
    ...
    expect(describeEvent(running, 'ml', NOW)).toMatchObject({
  ...
  it('distinguishes night sleep from a nap', () => {
    ...
    expect(describeEvent(night, 'ml', NOW).title).toBe('Night sleep')
  })

  it('names each diaper kind and needs no detail line', () => {
    expect(describeEvent(diaper(NOW, 'wet'), 'ml', NOW)).toMatchObject({
      category: 'diaper',
      title: 'Wet diaper',
      detail: '',
    })
    expect(describeEvent(diaper(NOW, 'mixed'), 'ml', NOW).title).toBe('Mixed diaper')
  })
})
